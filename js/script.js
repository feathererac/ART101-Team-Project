...
function sellItem(name, tokenAmount, button) {
    ...
    itemsSold += 1;
    winProbability = Math.max(0.1, winProbability - 0.17);
...
// Gambo!---->
const gamboGif = $("#gambo-gif");
    
function setGamboState(state) {
    const states = {
        regular: "./img/gamboRegular.gif",
        waiting: "./img/gamboWaiting.gif",
        blnt: "./img/gamboBLNT.gif",
        excited: "./img/gamboExcited.gif"
    };
    gamboGif.attr("src", states[state]);
}

// Default to regular state
setGamboState("regular");

// Change to waiting state in the shop
$("#desperate-options").on("show", function () {
    setGamboState("waiting");
});

// Excited state when hovering over shop items
$(".desperate-option button").hover(
    function () {
        setGamboState("excited");
    },
    function () {
        setGamboState("waiting"); 
    }
...
function onPlayerWin() {
    setGamboState("excited");
    setTimeout(() => setGamboState("regular"), 2000); 
}
function onPlayerLose() {
    setGamboState("waiting");
    setTimeout(() => setGamboState("regular"), 2000); 
 }
function outOfGambits(){
    setGamboState("blnt");
    setTimeout(() => setGamboState("regular"), 2000); 
}
